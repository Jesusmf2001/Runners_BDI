...
global {
    ...
    init {
	    ...
	    create runner number: 1 {
		    ...
		    // Adding all runners as friends except self
		    amigos <- (runner as list) - self;
		    
		    // Ensuring friendship is reciprocal
		    loop amigo over: amigos {
		        ask amigo {
		            self.amigos <- self.amigos + myself;
		        }
		    }
		    
		    write "Corredor " + string(id) + " amigo de: " + amigos;
		    
		    mi_casa <- any_location_in(one_of(building));
		    location <- any_location_in(one_of(the_graph.vertices));
		    write "Corredor " + string(id) + " creado.";
		}
            ...
            if (myself in self.amigos) {
                ...
                loop averia over: sus_averias {
                    ...
                    if (!exists) {
                        ...
                        write "Corredor " + string(id) + " aprendió sobre la avería en la fuente en " + string(predicado_averia.values["location_value"]);
                ...
                loop calle_cortada over: sus_calles_cortadas {
                    ...
                    if (!exists) {
                        ...
                        write "Corredor " + string(id) + " aprendió sobre la calle cortada en " + string(predicado_calle_cortada.values["vertice1"]);
                    }
                }
            }
        }
