...
species runner skills: [moving, fipa] control: simple_bdi {
    ...
    init {
        ...
        do add_belief(nada);
        write "Amigos de corredor " + string(id) + ": " + string(length(amigos));
        
        bool mochila <- flip(0.5);
        if mochila and nivel_choice = 2 {
            do add_belief(llevo_mochila);
            write "Corredor " + string(id) + " lleva mochila.";
        }
        if has_belief(llevo_mochila) {
            snack <- rnd(1,5);
            agua_mochila <- rnd(100.0,300.0);
            max_agua <- agua_mochila;
            write "Corredor " + string(id) + " lleva " + string(snack) + " snacks y " + string(agua_mochila) + " ml de agua.";
        }
    }
    
    reflex bajar_energia {
        energy <- energy - 0.01;
    ...
    plan elegir_punto intention: elegir_punto instantaneous: true {
        ...
        if (!empty(posibles_puntos)) {
            ...
            write "Corredor " + string(id) + " seleccionó un nuevo punto objetivo.";
        } else {
            proximo_punto <- ultimo_punto;  // Si no hay puntos posibles, volver al último punto
            write "Corredor " + string(id) + " no encontró un nuevo punto, regresando al último punto.";
        }
    ...
    plan correr intention: correr {
        if (proximo_punto = nil) {
            ...
            write "Corredor " + string(id) + " no tiene un punto objetivo, eligiendo nuevo punto.";
        } else {
            ...
            if (location = proximo_punto) {
                ...
                write "Corredor " + string(id) + " alcanzó el punto objetivo.";
            }
        }
    }
    
    plan elegir_fuente intention: elegir_fuente instantaneous: true {
        // Obtener la lista de fuentes conocidas que no están averiadas
    ...
    plan ir_casa intention: ir_casa {
        ...
        if (location = tarj.location) {
            ...
            write "Corredor " + string(id) + " llegó a casa.";
        }
    }
    ...
    plan beber_agua intention: beber_agua {
        if !(has_belief(estoy_en_fuente)) {
            if agua_mochila > 100 {
                ...
                agua_mochila <- 0.0;
                write "Corredor " + string(id) + " agotó el agua de la mochila. Sed: " + string(sed);
            }
        } else {
            sed <- 850.0;
            if has_belief(llevo_mochila) {
    ...
    rule beliefs: [tengo_sed, estoy_en_fuente] new_desire: beber_agua strength: 11.0;
